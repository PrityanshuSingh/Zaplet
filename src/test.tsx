...
function App() {
          ...
          <div className="prose prose-invert p-4 bg-gray-700 !text-white rounded-lg max-w-[95%] md:max-w-[80%] marker:text-white">
            {text === "" ? (
              <div className="flex gap-1 items-center">
                <Loader2
                  className={`mr-2 h-6 w-6 animate-spin ${progress ? "block" : "hidden"} `}
                />
                Generating Response...
              </div>
            ) : (
  ...
  const LikeComponent = (props: { url: string }) => {
    ...
    return (
      <Heart
        className={`ml-2 inline-block cursor-pointer ${isSaved ? "fill-red-500 stroke-red-500" : ""}`}
        onClick={() => {
          if (isSaved) {
  ...
  const [showPanelGroup, setShowPanelGroup] = useState(false);
  ...
  return (
    <>
        {showPanelGroup ? (
            ...
            <ResizablePanel defaultSize={75} minSize={45}>
              <Card className="shadow-lg relative rounded-none bg-gray-900 text-gray-100 border-none ">
                      ...
                      <div className="flex flex-row justify-center">
                        ...
                        >
                          <PopoverTrigger asChild className="lg:hidden">
                            <Button variant={"ghost"} size={"sm"}>
                              <NotebookPen
                                size={25}
                                className={`${screenWidth.current < 1024 ? "tour-saved-properties" : ""}`}
                              />
                            </Button>
                          </PopoverTrigger>
                          <PopoverContent
                        ...
                        >
                          {showMap ? (
                            ...
                            <MapIcon
                              size={25}
                              className={`${screenWidth.current < 1024 ? "tour-map" : ""}`}
                            />
                          )}
                        </Button>
                      </div>
                ...
                <CardContent className="p-0 flex flex-col h-dvh rounded-none">
                  ...
                  <form
                    className={`flex gap-2 bottom-5 max-w-[370px] md:max-w-screen-md w-full ${showMap ? "" : "fixed"} bg-gray-900 place-self-center items-end tour-input`}
                    onSubmit={(e) => {
                      UmamiTrack("chat", {
                        "chat-prompt": query,
                      });
                    ...
                    <div className="w-full flex flex-col gap-2 h-full">
                      {savedProperties.length > 0 &&
                        ...
                        query.lastIndexOf("@") === query.length - 1 && (
                          <ScrollArea
                            className={`${savedProperties.length > 3 ? "h-[200px]" : "h-fit"}  overflow`}
                          >
                ...
                <DialogContent className="bg-[#1F2937] border-none">
                  ...
                  <Form {...loginForm}>
                    ...
                    >
                      ...
                      >
                        <Loader2
                          className={`mr-2 h-4 w-4 animate-spin ${inProgress ? "block" : "hidden"}`}
                        />
                        Submit
                      </Button>
                      <a
            ...
            <HomePage onSearchClick={handleSearchClick} />
        )}
    </>
);
}
